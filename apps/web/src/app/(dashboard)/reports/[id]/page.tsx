          ...
          <div className="bg-white dark:bg-gray-800 p-8 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
            ...
            <div className="p-6 bg-primary/5 dark:bg-primary/10 rounded-2xl border border-primary/10">
              ...
              <p className="text-text-muted dark:text-gray-300 leading-relaxed">
                {report.analysis?.reportDescription || report.analysis?.patientSummary || 
                 'This report contains your medical test results. Please review the metrics below and consult with your healthcare provider for personalized medical advice.'}
              </p>
            </div>
            ...
            {report.analysis?.predictions && report.analysis.predictions.length > 0 && (
              <div className="mt-8 p-6 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-200 dark:border-amber-700/50">
                <div className="flex items-center gap-3 mb-4">
                  <AlertTriangle className="size-6 text-amber-600 dark:text-amber-500" />
                  <h4 className="font-bold text-lg text-amber-900 dark:text-amber-100">AI Health Predictions</h4>
                </div>
                
                <div className="mb-4 text-sm text-amber-800 dark:text-amber-200/80 bg-white/50 dark:bg-black/20 p-3 rounded-lg border border-amber-100 dark:border-amber-800">
                  <span className="font-bold">Disclaimer:</span> These predictions are generated by AI based on the extracted metrics. They are probabilistic and NOT a medical diagnosis. Always consult a doctor.
                </div>

                <ul className="space-y-3">
                  {report.analysis.predictions.map((prediction, i) => (
                    <li key={i} className="flex items-start gap-3 text-amber-900 dark:text-amber-100">
                      <span className="size-1.5 rounded-full bg-amber-500 mt-2 shrink-0"></span>
                      <span>
                        {prediction.replace(/^CAUTION: AI PREDICTION\s*[-–:]\s*/i, '')}
                      </span>
                    </li>
                  ))}
                </ul>
